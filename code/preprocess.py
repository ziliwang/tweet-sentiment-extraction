...
sentiment_to_id = {'positive': 1313, 'negative': 2430, 'neutral': 7974}
...
def annotate(moses_tokenize, tokenizer, text, selected_text):
    ...
    tokens, offsets, tokens_ids, moses_offsets = get_offsets(moses_tokenize, tokenizer, text.replace('`', "'").replace('´', "'").replace("ï¿½", "***").replace('â', 'a'))
...
def process_test(df, moses_tokenize, tokenizer):
    output = []
    for idx, r in df.iterrows():
        item = {}
        text = ' '.join(r.text.split()).lower()
        tokens, offsets, tokens_ids, moses_offsets = get_offsets(moses_tokenize, tokenizer, text.replace('`', "'").replace('´', "'").replace("ï¿½", "***").replace('â', 'a').replace('¡', 'i'))
        item['text'] = text
        item['tokens_id'] = tokens_ids
        item['offsets'] = offsets
        item['id'] = r.textID
        item['sentiment'] = sentiment_to_id[r.sentiment]
        output.append(item)
    return output
...
def main(vocab, merges, data_path, save_path):
    ...
    tokenizer = ByteLevelBPETokenizer(vocab, merges, lowercase=True)
    ...
    for line, row in train.iterrows():
        ...
        try:
            ...
            print(s)
        ann['sentiment'] = sentiment_to_id[row.sentiment]
        ann['id'] = row.textID
        dataset.append(ann)
    ...
    test = process_test(pd.read_csv(os.path.join(data_path, 'test.csv')), moses_token_func, tokenizer)
    joblib.dump({'train': dataset, 'test': test}, save_path, compress='zlib')
